...
const ListSectionFive: React.FC = () => {
  ...
  async function handlePrefetchSectioFive(sectionFiveId: string) {
    await queryClient.prefetchQuery(['sectionFive', sectionFiveId], async () => {
      const response = await api.get(`/sectionFive/${sectionFiveId}`);
        ...
        <Flex>
          ...
          <Flex flex={1} justify="right" align="center">
            <Flex borderRadius={10}>
              ...
              <Link as={RouterLink} to={'/create-section-five'} bg={bg} mr={1} p={2} borderRadius={10}>
                <FaPlus />
              </Link>
            </Flex>
          </Flex>
                  ...
                  {data?.sections.map(section => (
                    <Tr key={section.id}>
                      <Td paddingTop="2" paddingBottom="2" minW='8rem'>
                        <Box>
                          <Link onMouseEnter={() => handlePrefetchSectioFive(section.id)}>
                      ...
                      <Td paddingTop="2" paddingBottom="2" maxW='1rem'>
                        <Flex justify='center' align='center' gap='1'>
                          ...
                          <Button onClick={() => deleteSectionFive(section.id)} p={0}>
