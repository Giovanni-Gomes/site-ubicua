...
export async function getContracts(page: number, take: number, searchQuery?: string): Promise<GetContractResponse> {
  ...
  const contracts = data.contracts.map((contract: ContractProps) => ({
    ...
    date_end: contract.date_end && new Date(contract.date_end).toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    }),
    createdAt: contract.created_at && new Date(contract.created_at).toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    }),
...
export async function getOneContractById(id: string): Promise<ContractProps> {
  ...
  const contract = {
    ...
    created_at: result.data.created && new Date(result.data.date_end).toISOString().slice(0, 10),
  }
  return contract
}
...
export function useContracts(page: number, take: number, searchQuery?: string) {
  return useQuery(['contracts', page, take, searchQuery], () => getContracts(page, take), {
