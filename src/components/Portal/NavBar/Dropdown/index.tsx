        ...
        <div className="menu">
          {subtitle.map((sb, key) => (
            sb.link ?
              ...
              : <div className={sb.name === "Secções" ? "space expand" : "expand"} key={key.toString()} onClick={() => setActiveMenu(sb.name)}>{sb.name}<CaretRight size={18} /> </div>
          ))}
        </div>
      </CSSTransition>
      <CSSTransition in={activeMenu === 'Acompanhamentos'} unmountOnExit timeout={500} classNames="menu-secondary" onEnter={calcHeight}>
        <div className="menu">
          <div className="space" onClick={() => setActiveMenu('main')}><CaretLeft size={18} />Voltar</div>
